	...
	Label(tsparams.LabelReinstall), func() {
		...
		It("through siteconfig operator is successful in an IPv4 environment with DHCP networking",
			reportxml.ID("no-testcase"), func() {
				...
				scoConfig, err := configmap.Pull(APIClient, "siteconfig-operator-configuration", tsparams.RACMNamespace)
				...
				clusterInstace.Definition.Spec.Reinstall.Generation = "regnerate1"
