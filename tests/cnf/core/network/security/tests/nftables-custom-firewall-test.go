...
var _ = Describe("nftables", Ordered, Label(tsparams.LabelNftablesTestCases), ContinueOnFailure, func() {
	...
	BeforeAll(func() {
		...
		testPodWorker1 = createTestPodOnWorkers(cnfWorkerNodeList[1].Definition.Name, portNum8888)
		...
		routeMap, err = frrconfig.BuildRoutesMapWithSpecificRoutes(testPodList, cnfWorkerNodeList, ipv4SecurityIPList)
	...
	Context("custom firewall", func() {
		...
		It("Verify the creation of a new custom node firewall NFTables table with an ingress rule",
			reportxml.ID("77412"), func() {
				...
				err := cmd.ICMPConnectivityCheck(masterPod, ipv4NodeAddrList, interfaceNameNet1)
				...
				By("Verify egress TCP traffic over port 8888 between the test Pod on the worker and the master pod")
...
func createExternalNad(name string) {
	...
	_, err = define.CreateNadWithMasterPlugin(APIClient, name, tsparams.TestNamespaceName, macVlanPlugin)
...
func setupRemoteMultiHopTest(
	...
	createExternalNad(externalMacVlanNADName)

	By("Creating External NAD for hub FRR pods")
	createExternalNad(nadHubName)

	By("Creating static ip annotation for hub0")

	hub0BRstaticIPAnnotation := define.CreateStaticIPAnnotations(externalMacVlanNADName,
		nadHubName,
	...
	hub1BRstaticIPAnnotation := define.CreateStaticIPAnnotations(externalMacVlanNADName, nadHubName,
	...
	frrConfigMapStaticRoutes := defineConfigMapWithStaticRouteAndNetwork(hubIPv4ExternalAddresses,
		frrconfig.RemovePrefixFromIPList(ipv4NodeAddrList))
	masterConfigMap := createFrrConfigMap("master-configmap", frrConfigMapStaticRoutes)

	masterStaticIPAnnotation := createStaticIPAnnotations(externalMacVlanNADName, nadHubName,
...
func addDeleteStaticRouteOnWorkerNodes(testPodList []*pod.Builder, routeMap map[string]string, routeAction,
	...
	for _, testPod := range testPodList {
		_, err := frrconfig.SetStaticRoute(testPod, routeAction, hubIPv4Network, "", routeMap)
