	...
	ContinueOnFailure, func() {
		...
		BeforeAll(func() {
			...
			if !isSecureBootEnabled {
				configureSecureBoot(bmcClient, "enable")
			}
		})
		...
		AfterAll(func() {
			...
			if isSecureBootEnabled {
				configureSecureBoot(bmcClient, "disable")
			}
...
func configureSecureBoot(bmcClient *bmc.BMC, action string) {
	switch action {
	case "enable":
		By("Enabling SecureBoot")
		Eventually(bmcClient.SecureBootEnable).WithTimeout(tsparams.DefaultTimeout).
	...
	case "disable":
		By("Disabling SecureBoot")
		Eventually(bmcClient.SecureBootDisable).WithTimeout(tsparams.DefaultTimeout).
			WithPolling(tsparams.PollingIntervalBMC).ShouldNot(HaveOccurred(),
			"SecureBootDisable should not return an error")
	default:
		Fail(fmt.Sprintf("Invalid action provided: %s. Allowed actions are 'enable' or 'disable'", action))
	}

	By("Restarting node")
	Eventually(bmcClient.SystemPowerCycle).WithTimeout(tsparams.DefaultTimeout).
		WithPolling(tsparams.PollingIntervalBMC).ShouldNot(HaveOccurred(),
		"Power cycling node should succeed")

	By("Waiting for the cluster becomes stable")
...
func createBMCClient() *bmc.BMC {
	bmcHosts, err := NetConfig.GetBMCHostNames()
	Expect(err).ToNot(HaveOccurred(), "Failed to get BMC host names")

	bmcUser, err := NetConfig.GetBMCHostUser()
	Expect(err).ToNot(HaveOccurred(), "Failed to get BMC host user")

	bmcPass, err := NetConfig.GetBMCHostPass()
	Expect(err).ToNot(HaveOccurred(), "Failed to get BMC host password")

	return bmc.New(bmcHosts[0]).
		WithRedfishUser(bmcUser, bmcPass).
		WithRedfishTimeout(5*time.Minute).
		WithSSHUser(bmcUser, bmcPass)
}
