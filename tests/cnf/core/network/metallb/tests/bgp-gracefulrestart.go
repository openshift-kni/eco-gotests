...
var _ = Describe("BGP GR", Ordered, Label(tsparams.LabelGRTestCases), ContinueOnFailure, func() {
	var (
	...
	Context("without bfd", func() {
		BeforeEach(func() {
			...
			By("Configuring BGP peers with GR disabled")
		...
		It("Verify basic functionality with externalTrafficPolicy set to Cluster", reportxml.ID("77077"), func() {
			...
			bgpRoutes := getRoutes(frrPod)
			Expect(len(bgpRoutes)).To(Equal(0), "Routes were not removed as expected")
		})

	})
})

func getRoutes(masterNodeFRRPod *pod.Builder) map[string][]frr.Route {
	Eventually(
		frr.GetBGPStatus, time.Minute, tsparams.DefaultRetryInterval).
		WithArguments(masterNodeFRRPod, strings.ToLower(netparam.IPV4Family)).ShouldNot(BeNil())
...
func gracefulRestartTest(
	...
	Expect(err).To(HaveOccurred(), "Unexpectedly succeeded in curling the Nginx pod")
	...
	bgpRoutes = getRoutes(frrPod)
	Expect(len(bgpRoutes)).To(Equal(1), "No routes were added")
	...
	bgpRoutes = getRoutes(frrPod)
	Expect(len(bgpRoutes)).To(Equal(1), "Routes were removed with GR enabled")
...
func validateCurlToNginxPod(frrPod *pod.Builder, srcIP, destIP string) {
	Eventually(func() error {
		...
		return err
	}, time.Minute, tsparams.DefaultRetryInterval).Should(BeNil(), "Failed to curl nginx pod")
}
