	...
	Context("with bfd", func() {
		...
		Context("single hop", func() {
			...
			It("Test Compatibility with BFD", reportxml.ID("77101"), func() {
				gracefulRestartTestWithBFD("Cluster", addressPool[0], ipv4MetalLbIPList[0], frrPod, ipAddressPool, workerLabelMap)
			})
		})
		Context("multi hop", func() {
			...
			It("Scenario with BFD", reportxml.ID("77125"), func() {
				gracefulRestartTestWithBFD("Local", addressPool[0], masterClientPodIP, frrPod, ipAddressPool, workerLabelMap)
			})
		})
