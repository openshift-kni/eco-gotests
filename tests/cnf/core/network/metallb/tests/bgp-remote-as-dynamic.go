...
func setupBGPRemoteASTestCase(hubIPv4ExternalAddresses, externalAdvertisedIPv4Routes,
	...
	By("Waiting until the new frr-k8s daemonset is in Ready state.")

	frrk8sDS, err := daemonset.Pull(
		APIClient, tsparams.FrrDsName, NetConfig.MlbOperatorNamespace)
	Expect(err).ToNot(HaveOccurred(), "Fail to pull frr-k8s daemonset")
	Expect(frrk8sDS.IsReady(30*time.Second)).To(BeTrue(),
		"frr-k8s daemonset deployment is not ready")
