...
var _ = Describe("BGP remote-dynamicAS", Ordered, Label(tsparams.LabelDynamicRemoteASTestCases),
	ContinueOnFailure, func() {
		...
		BeforeAll(func() {
			...
			var cnfSingleWorkerNodeList = []*nodes.Builder{cnfWorkerNodeList[0]}
			workerLabelMap, workerNodeList = setWorkerNodeListAndLabelForBfdTests(cnfSingleWorkerNodeList, metalLbTestsLabel)
