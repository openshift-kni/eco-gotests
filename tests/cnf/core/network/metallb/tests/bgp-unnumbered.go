...
var _ = Describe("BGP Unnumbered", Ordered, Label(tsparams.LabelBGPUnnumbered),
	ContinueOnFailure, func() {
		...
		BeforeAll(func() {
			...
			By(fmt.Sprintf(
				"nmstate policy configuring ethernet interface %s on %s with an IP address",
		...
		It("Verify iBGP peering established with BGP Unnumbered",
			reportxml.ID("80393"), func() {
				...
				createBGPPeerUnnumberedAndVerifyIfItsReady(tsparams.BgpPeerName1, tsparams.BgpPeerDynamicASiBGP,
					interfacesUnderTest[0], bfdProfileName, tsparams.LocalBGPASN, 0, false,
		...
		It("Verify eBGP peering established and advertised IPv4 prefixes received by the external FRR pod",
			reportxml.ID("80394"), func() {
				...
				frrPod := createFrrPod(
					workerNodeList[1].Object.Name, frrConfigMap.Definition.Name, []string{}, frrStaticAnnotation)
				By(fmt.Sprintf("Work around to lab infrastructure issue reset the node interface %s", interfacesUnderTest[0]))
				_, err := cluster.ExecCmdWithStdout(
					APIClient, fmt.Sprintf("ip link set dev %s down && ip link set dev %s up", interfacesUnderTest[0],
						interfacesUnderTest[0]),
					metav1.ListOptions{LabelSelector: labels.Set(map[string]string{netparam.LabelHostName: cnfWorkerNodeList[0].Definition.Name}).String()})
				Expect(err).ToNot(HaveOccurred(), "Failed to reset interface")
		...
		It("Verify iBGP peering established with configured peerASN and unnumbered",
			reportxml.ID("80395"), func() {
				...
				createBGPPeerUnnumberedAndVerifyIfItsReady(tsparams.BgpPeerName1, "",
					interfacesUnderTest[0], bfdProfileName, tsparams.LocalBGPASN, tsparams.LocalBGPASN,
