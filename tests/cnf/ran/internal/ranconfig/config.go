...
func (ranconfig *RANConfig) newHubConfig(configFile string) {
	...
	ranconfig.HubConfig.ACMVersion, err = ranhelper.GetOperatorVersionFromCsv(
		ranconfig.HubConfig.HubAPIClient, ranparam.AcmOperatorName, ranparam.AcmOperatorNamespace)
	if err != nil {
		glog.V(ranparam.LogLevel).Infof("Failed to get ACM version from hub: %v", err)
	}

	ranconfig.HubConfig.TALMVersion, err = ranhelper.GetOperatorVersionFromCsv(
		ranconfig.HubConfig.HubAPIClient, ranparam.TalmOperatorHubNamespace, ranparam.OpenshiftOperatorNamespace)
	if err != nil {
	...
	ranconfig.HubConfig.ZTPVersion, err = ranhelper.GetZTPVersionFromArgoCd(
		ranconfig.HubConfig.HubAPIClient, ranparam.OpenshiftGitopsRepoServer, ranparam.OpenshiftGitops)
	if err != nil {
