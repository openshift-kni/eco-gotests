...
func SetCPUFreq(
	...
	err = wait.PollUntilContextTimeout(
		context.TODO(), 2*time.Second, 1*time.Minute, true, func(ctx context.Context) (bool, error) {
			...
			cmdOut, err := cluster.ExecCommandOnSNOWithRetries(Spoke1APIClient, 3, spokeCommandIsolatedCPUs)
			...
			// Get current isolated core frequency from spoke cluster and compare to desired frequency
			cmdOut, err = cluster.ExecCommandOnSNOWithRetries(Spoke1APIClient, 3, spokeCommandReservedCPUs)
