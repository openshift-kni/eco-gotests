		...
		Context("precache OCP with image", func() {
			...
			BeforeEach(func() {
				...
				if excludedPreCacheImage != "" {
					...
					_ = cluster.ExecCmdWithRetries(Spoke1APIClient, 3, tsparams.MasterNodeSelector, imageDeleteCommand)
				}
			})
			...
			It("tests custom image precaching using a PreCachingConfig CR", reportxml.ID("64746"), func() {
				...
				_ = cluster.ExecCmdWithRetries(Spoke1APIClient, 3, tsparams.MasterNodeSelector, ptpImageDeleteCmd)
