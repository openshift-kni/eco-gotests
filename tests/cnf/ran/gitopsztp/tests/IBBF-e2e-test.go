...
var _ = Describe(
	...
	Label(tsparams.LabelIBBFe2e), func() {
		...
		It("tests HW replacement via IBBF", reportxml.ID("78333"), func() {
			...
			log.Printf("Debug: testGitPath=%s", testGitPath)
			log.Printf("Debug: gitDetails.Path=%s", gitDetails.Path)
			log.Printf("Debug: testGitPath+tsparams.ZtpKustomizationPath=%s", testGitPath+tsparams.ZtpKustomizationPath)
