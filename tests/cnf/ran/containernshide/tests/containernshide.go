...
var _ = Describe("Container Namespace Hiding", Label(tsparams.LabelContainerNSHideTestCases), func() {
	It("should not have kubelet and crio using the same inode as systemd", reportxml.ID("53681"), func() {
		...
		systemdInodes, err := cluster.ExecCmdWithStdoutWithRetries(Spoke1APIClient, 3, "readlink /proc/1/ns/mnt")
		...
		crioInodes, err := cluster.ExecCmdWithStdoutWithRetries(Spoke1APIClient, 3, "readlink /proc/$(pidof crio)/ns/mnt")
