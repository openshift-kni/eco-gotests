...
var (
	...
	serverDeploymentLabelMap = map[string]string{"rds-app": "rootless-dpdk-server"}
...
func defineDPDKContainer(
	...
	if err != nil {
		glog.V(100).Infof("Fail to define server container %s due to %v", cName, err)

		return nil, fmt.Errorf("fail to define server container %s due to %w", cName, err)
	}
...
func retrieveClientDPDKPod(apiClient *clients.Settings, podNamePattern, podNamespace string) (*pod.Builder, error) {
	...
	if err != nil {
		glog.V(100).Infof("The pod-level bonded pod %s in namespace %s is not in Ready condition: %v",
			podNamePattern, podNamespace, err)

		return nil, fmt.Errorf("the pod-level bonded pod %s in namespace %s is not in Ready condition: %w",
			podNamePattern, podNamespace, err)
	}
...
func checkRxOutputRateForInterfaces(
	...
	for interfaceName, TrafficRate := range interfaceTrafficRateMap {
		...
		if TrafficRate == 5000 && currentRate > TrafficRate || TrafficRate != 5000 && currentRate < TrafficRate {
...
func getPCIAddressListFromSrIovNetworkName(podNetworkStatus string) ([]string, error) {
	...
	for _, networkAnnotation := range podNetworkStatusType {
		if strings.Contains(networkAnnotation.Name, dpdkNetworkOne) {
...
func VerifyRootlessDPDKOnTheSameNodeSingleVFMultipleVlans(ctx SpecContext) {
	...
	Eventually(
		isPciAddressAvailable, waitTimeout, retryInterval).WithArguments(clientPod).Should(BeTrue())
