...
func VerifyAllocatedNodeDoesNotExist(allocatedNode *oran.AllocatedNodeBuilder, waitGroup *sync.WaitGroup, ctx SpecContext) {
	defer waitGroup.Done()
	defer GinkgoRecover()
...
func VerifyNARExistsInNamespace(nodePoolName string, nsName string) *oran.NARBuilder {
	...
	Expect(err).ToNot(HaveOccurred(), "Failed to pull node allocation request %s from namespace %s: %v", nodePoolName, nsName, err)
